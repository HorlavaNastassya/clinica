...
"""The 'clinicaml' executable command line, installed with the clinicaml package, calls this module.
...
class ClinicaClassLoader:
    """Load pipelines from a custom locations (general from $HOME/clinicaml)."""
...
def execute():
    ...
    # Nice traceback when clinicaml crashes
    ...
    parser._positionals.title = (
        f"{Fore.YELLOW}clinicaml expects one of the following keywords{Fore.RESET}"
    )
    ...
    parser.add_argument(
        ...
        "--logname",
        dest="logname",
        default="clinicaml.log",
        metavar=("file.log"),
        help="Define the log file name (default: clinicaml.log)",
    )
    ...
    """
    # fmt: off
    from clinicaml.engine import CmdParser
    from clinicaml.pipelines.deeplearning_prepare_data.deeplearning_prepare_data_cli import DeepLearningPrepareDataCLI
    from clinicaml.pipelines.dwi_connectome.dwi_connectome_cli import DwiConnectomeCli
    from clinicaml.pipelines.dwi_dti.dwi_dti_cli import DwiDtiCli
    from clinicaml.pipelines.dwi_preprocessing_using_phasediff_fieldmap.dwi_preprocessing_using_phasediff_fieldmap_cli import DwiPreprocessingUsingPhaseDiffFieldmapCli
    from clinicaml.pipelines.dwi_preprocessing_using_t1.dwi_preprocessing_using_t1_cli import DwiPreprocessingUsingT1Cli
    from clinicaml.pipelines.machine_learning_spatial_svm.spatial_svm_cli import SpatialSVMCLI
    from clinicaml.pipelines.pet_linear.pet_linear_cli import PETLinearCLI
    from clinicaml.pipelines.pet_surface.pet_surface_cli import PetSurfaceCLI
    from clinicaml.pipelines.pet_surface.pet_surface_longitudinal_cli import PetSurfaceLongitudinalCLI
    from clinicaml.pipelines.pet_volume.pet_volume_cli import PETVolumeCLI
    from clinicaml.pipelines.statistics_surface.statistics_surface_cli import StatisticsSurfaceCLI
    from clinicaml.pipelines.statistics_volume.statistics_volume_cli import StatisticsVolumeCLI
    from clinicaml.pipelines.statistics_volume_correction.statistics_volume_correction_cli import StatisticsVolumeCorrectionCLI
    from clinicaml.pipelines.t1_freesurfer.t1_freesurfer_cli import T1FreeSurferCLI
    from clinicaml.pipelines.t1_freesurfer_longitudinal.t1_freesurfer_longitudinal_cli import T1FreeSurferLongitudinalCLI
    from clinicaml.pipelines.t1_freesurfer_longitudinal.t1_freesurfer_longitudinal_correction_cli import T1FreeSurferLongitudinalCorrectionCLI
    from clinicaml.pipelines.t1_freesurfer_longitudinal.t1_freesurfer_template_cli import T1FreeSurferTemplateCLI
    from clinicaml.pipelines.t1_linear.t1_linear_cli import T1LinearCLI
    from clinicaml.pipelines.t1_volume.t1_volume_cli import T1VolumeCLI
    from clinicaml.pipelines.t1_volume_create_dartel.t1_volume_create_dartel_cli import T1VolumeCreateDartelCLI
    from clinicaml.pipelines.t1_volume_dartel2mni.t1_volume_dartel2mni_cli import T1VolumeDartel2MNICLI
    from clinicaml.pipelines.t1_volume_existing_template.t1_volume_existing_template_cli import T1VolumeExistingTemplateCLI
    from clinicaml.pipelines.t1_volume_parcellation.t1_volume_parcellation_cli import T1VolumeParcellationCLI
    from clinicaml.pipelines.t1_volume_register_dartel.t1_volume_register_dartel_cli import T1VolumeRegisterDartelCLI
    from clinicaml.pipelines.t1_volume_tissue_segmentation.t1_volume_tissue_segmentation_cli import T1VolumeTissueSegmentationCLI
    # fmt: on

    pipelines = ClinicaClassLoader(baseclass=CmdParser, extra_dir="pipelines").load()
    # The order in pipelines var will the same when typing `clinicaml run`
    ...
    run_parser._positionals.title = (
        f"{Fore.GREEN}clinicaml run expects one of the following pipelines{Fore.RESET}"
    )
    ...
    """
    # fmt: off
    from clinicaml.iotools.converters.adni_to_bids.adni_to_bids_cli import \
        AdniToBidsCLI
    from clinicaml.iotools.converters.aibl_to_bids.aibl_to_bids_cli import \
        AiblToBidsCLI
    from clinicaml.iotools.converters.nifd_to_bids.nifd_to_bids_cli import \
        NifdToBidsCLI
    from clinicaml.iotools.converters.oasis_to_bids.oasis_to_bids_cli import \
        OasisToBidsCLI
    ...
    convert_parser._positionals.title = f"{Fore.YELLOW}clinicaml convert expects one of the following datasets{Fore.RESET}"
    ...
    io_parser._positionals.title = f"{Fore.YELLOW}clinicaml iotools expects one of the following BIDS/CAPS utilities{Fore.RESET}"
    ...
    """
    from clinicaml.engine import CmdParser
    from clinicaml.pipelines.t1_freesurfer.t1_freesurfer_visualizer import (
    ...
    visualize_parser._positionals.title = f"{Fore.YELLOW}clinicaml visualize expects one of the following pipelines{Fore.RESET}"
    ...
    generate_parser._positionals.title = (
        f"{Fore.YELLOW}clinicaml generate expects one of the following tools{Fore.RESET}"
    )
    generate_parser._optionals.title = OPTIONAL_TITLE

    from clinicaml.engine.template import CmdGenerateTemplates
    ...
    try:
        ...
        if args.version:
            ...
            print(f"Clinica version is: {clinicaml.__version__}")
            exit(0)
        if unknown_args:
    ...
    if "run" in args and hasattr(args, "func") is False:
        # Case when we type `clinicaml run` on the terminal
        run_parser.print_help()
        exit(0)
    elif "convert" in args and hasattr(args, "func") is False:
        # Case when we type `clinicaml convert` on the terminal
        convert_parser.print_help()
        exit(0)
    elif "iotools" in args and hasattr(args, "func") is False:
        # Case when we type `clinicaml iotools` on the terminal
        io_parser.print_help()
        exit(0)
    elif "visualize" in args and hasattr(args, "func") is False:
        # Case when we type `clinicaml visualize` on the terminal
        visualize_parser.print_help()
        exit(0)
    elif "generate" in args and hasattr(args, "func") is False:
        # Case when we type `clinicaml generate` on the terminal
        generate_parser.print_help()
        exit(0)
    elif args is None or hasattr(args, "func") is False:
        # Case when we type `clinicaml` on the terminal
        parser.print_help()
        exit(0)

    import clinicaml.utils.stream as var

    var.clinica_verbose = args.verbose

    if args.verbose is False:
        """
        Enable only cprint(msg) --> clinicaml print(msg)
