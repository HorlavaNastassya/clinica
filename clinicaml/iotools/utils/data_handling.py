...
def center_all_nifti(bids_dir, output_dir, modality, center_all_files=False):
    ...
    from clinicaml.utils.exceptions import ClinicaBIDSError
    from clinicaml.utils.inputs import check_bids_folder
...
def check_relative_volume_location_in_world_coordinate_system(
    ...
    Args:
        ...
        modality: string that must be used in argument of: clinicaml iotools bids --modality MODALITY (used in potential
                warning message)
    """
    import sys
    ...
    if len(pairs_with_problems) > 0:
        ...
        warning_message += (
            ...
            f'{Fore.BLUE}clinicaml iotools center-nifti {abspath(bids_dir)} {abspath(bids_dir)}_centered --modality "{modality}"{Fore.RESET}\n\n'
            f"{Fore.YELLOW}You will find more information on the command by typing {Fore.RESET}"
            f"{Fore.BLUE}clinicaml iotools center-nifti{Fore.RESET}"
...
def check_volume_location_in_world_coordinate_system(
    ...
    If yes, we warn the user of this problem, and propose him to exit clinicaml in order for him to run:
        clinicaml iotools center-nifti ...
    ...
    if len(list_non_centered_files) > 0:
        ...
        cmd_line = (
            f"{Fore.BLUE}\n"
            f'clinicaml iotools center-nifti {abspath(bids_dir)} {abspath(bids_dir)}_centered --modality "{modality}"'
            f"\n\n{Fore.YELLOW}"
        )
