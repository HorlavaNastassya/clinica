...
def check_caps_folder(caps_directory):
    ...
    from clinicaml.utils.exceptions import ClinicaCAPSError
...
def clinica_file_reader(
    ...
    from clinicaml.utils.exceptions import ClinicaBIDSError, ClinicaCAPSError
    ...
    if len(error_encountered) > 0 and raise_exception is True:
        ...
        if "needed_pipeline" in information.keys():
            if information["needed_pipeline"]:
                error_message += (
                    f"{Fore.YELLOW}Please note that the following clinicaml pipeline(s) must "
    ...
    if len(current_glob_found) != 1 and raise_exception is True:
        ...
        if len(current_glob_found) == 0:
            ...
            error_string += (
                ...
                f"Please note that the following clinicaml pipeline(s) must have run to obtain these files: "
...
def get_file_from_server(remote_file, cache_path=None):
    ...
    Args:
        ...
        cache_path (str): (default: ~/.cache/clinicaml/data)
    ...
    if cache_path:
        ...
        cache_clinica = os.path.join(home, ".cache", "clinicaml", "data")
